...
- 中文命名的资源在部分平台构建/导出路径下可能出问题——稳妥起见统一用英文小写 + 下划线命名。
...
**总计预估约 9-10 小时（一个人全职一天的工作量）**。你会发现在实际制作中——有些项目比你预估的快（idle 动画 15 分钟就画好了），有些比你预估的慢（Boss 死亡动画花了 2 小时）。这是正常的——管线存在的意义就是**帮你记录"实际 vs 预估"的偏差，让你下一次规划时更准确**。
