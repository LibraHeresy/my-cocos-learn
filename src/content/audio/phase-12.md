...
**为什么 MP3 循环有缝？** MP3 编码会在首尾插入少量填充（通常几十毫秒量级，LAME 可用 gapless 元数据消除）——但即便如此，MP3 做无缝循环仍容易有爆音（loop 播放时首尾的填充静音会形成"咔嗒"停顿），所以游戏循环 BGM 首选 OGG。OGG Vorbis 在设计时就考虑了无缝循环——它的编码架构允许精确指定循环点，没有填充静音。
