...
你打开一个 App——闪屏→首页→详情→返回。这个"页面栈"你每天都在经历，但你有没有想过：**那个闪屏在干嘛？** 它不是装饰——它在加载资源。游戏也一样：Logo→主菜单→游戏中→结算。但游戏多了一个问题：加载。一个场景可能包含几十 MB 的纹理、音频、动画——如果你让玩家在"开始游戏"按钮前等 5 秒，60% 的人会直接关掉。这一节讲怎么像 Vue Router 一样设计场景架构，同时让玩家感觉不到加载。

## ⏳ 闪屏到底在干什么？——一个前端工程师都能理解的加载之谜

你有没有注意过——启动一个大型 App 时，闪屏会停留 1-3 秒。这 3 秒不是设计师觉得 Logo 好看想多给你看看。这 3 秒里发生的事：

1. **初始化引擎**（加载 JS 运行时、WebGL 上下文）
2. **加载全局资源**（纹理图集、音频文件、字体）
3. **初始化全局 Manager**（GameManager、AudioManager）
4. **验证本地数据**（存档、设置、登录状态）

这和 Web 前端的"首屏加载"是同一个问题——只不过在游戏里，加载失败不是白屏，是**纹理缺失导致粉红色方块**（Unity 经典 Bug）。解决之道也是一样的：**懒加载 + 预加载 + 骨架屏（游戏里叫 Loading Screen）**。

## 🗺️ 场景结构：像 Vue Router 的路由树一样设计

单场景 = 所有东西挤在一起。多场景 = 像 Vue Router 一样组织结构——启动场景加载资源，菜单场景选关卡，游戏场景打飞机，结算场景看分数。每一个场景就是一个"页面"，场景之间的跳转就是"路由切换"：
...
为什么需要 Boot 场景？因为 `director.loadScene` 是异步的——如果在 MainMenu 中才开始加载 30MB 的游戏资源，玩家会在菜单页面等好几秒。Boot 场景在后台静默加载，加载完自动跳转。这和 Webpack 的 Code Splitting + Lazy Loading 是一样的思路：**先让用户看到东西，然后在后台加载重的部分**。

## 🔄 场景切换 + 数据传递：router.push + Pinia persist

切换场景时怎么传递数据？和前端一样，有两种主要思路：
...
## ✨ 转场动画：游戏世界的 Vue Transition

场景直接切过去——黑屏半秒——新场景出现。这种体验像是"游戏卡了"。加一个 0.5 秒的淡入淡出转场，体验立刻变成"场景流畅切换"。这和 Vue 的 `<Transition>` 组件的设计意图完全一致：
...
- **GTA V 和《塞尔达传说：旷野之息》的开放世界流式加载——场景管理的终极形态：** 2013 年，《GTA V》的 Los Santos 地图大约 127 平方公里，包含超过 20 万种不同的模型和纹理。如果一次性加载，需要约 80GB 内存——没有消费级 PC 装得下。Rockstar 的解决方案是**World Streaming**：以玩家为中心的 300 米半径内加载高精度模型，300-500 米加载低精度版本，500 米外只加载地形轮廓。玩家开车时，加载系统需要在每一帧判断"哪些区域该加载了、哪些该卸载了"——而且不能让玩家感觉到建筑在"凭空出现"。2017 年《旷野之息》更进一步——它在玩家向地平线眺望时提前加载远景的地形和敌人营地。你在开发微信小游戏时不需要 World Streaming（场景太小了），但理解"加载不是一次性完成的"这个思想，会让你在设计 Boot 场景时更有想象力。
- **Unity Addressable Assets vs Cocos Bundle——不止是"加载资源"：** 2018 年，Unity 推出了 Addressable Assets 系统，核心理念是"资源不再绑定在场景上，而是一个独立的、可寻址的资产包"。这和 Cocos Creator 的 Bundle 系统（2019 年推出）目标一致——允许你按需加载资源包，而不是在启动时一次性加载所有东西。但两个系统背后都藏着一个更深的哲学问题：**如何管理资源的生命周期**。哪些资源是全局永驻的（UI 材质、音效管理器）？哪些是场景限定的（Boss 的纹理、BGM）？哪些是可以随时释放的（上一关的场景数据）？这和前端 Webpack 的 Code Splitting（`import()`动态导入 + chunk 分割）完全是一种思路——只是游戏的资源体积通常是前端的 10-100 倍，所以做错更疼。
- **《战神》（2018）的"一镜到底"——当场景切换变成艺术：** 2018 年 Santa Monica Studio 的《战神》做出了一个惊人的技术决定：全游戏**没有任何场景切换**。没有加载画面、没有黑屏、没有"传送点"——从开场到结尾，摄像机始终跟随着奎托斯，像一个连续的长镜头。为了实现这一点，技术团队在**玩家走路的 2 秒内**，在后台卸载身后刚走过的场景、加载面前即将到达的场景——玩家在上一个区域战斗，下一个区域的 Boss 已经在内存里等着了。这种"无缝加载"是目前游戏场景管理的巅峰之作，但它的核心原理和你 Boot.scene 的预加载是一模一样的：**提前加载，让用户感觉不到等待**。

## 🔧 动手：创建三场景 + 淡入淡出转场 + 跨场景数据传递

在你的 Cocos 项目中新建三个场景文件，实现完整的场景流转。每个场景都绑定对应的脚本：
...
**验证：** 运行后你应该看到 Boot → MainMenu（自动跳转）→ 点击按钮 → 屏幕变黑 0.5s → Game 场景出现并淡入 → 游戏结束 → GameOver 场景显示分数。整个流程像完整游戏一样流畅。

## ✅ 自测清单

学完这一节，你应该能回答：

1. 为什么要把游戏拆成多个场景，而不是在一个场景里切换不同的预制体/Panel？从资源加载（内存占用）、代码组织（关注点分离）和团队协作（多人并行开发）三个角度思考。
2. `director.loadScene` 是异步的——场景加载过程中旧场景还在运行吗？如果玩家在加载过程中疯狂点击按钮会发生什么？如何用一个"加载中"遮罩来阻止这种边界情况？（提示：这和 Vue Router 的 `beforeEach` + Loading 状态是同一个思路）
3. 跨场景传递数据有全局 Manager 和 loadScene 回调两种方式。它们各自适合什么场景？如果 GameOver 场景需要读取"玩家使用的角色皮肤 ID"和"本局击杀数"——你会用哪种方式？为什么？
4. 转场动画在功能上不是必需的——没有淡入淡出照样能切换场景。为什么几乎每个商业游戏都有转场动画？它解决的是设计问题还是技术问题？
