...
<template>
  <PhaseLayout :phase="11" title="子弹系统与发射模式" duration="1-2 天">
    <ConceptBlock icon="🧭" title="本节定位">
      <p>你玩过《雷电》吗？那满屏的子弹不光是要躲的——它们是一种<strong>几何美学</strong>。从单发到手枪到霰弹枪到追踪导弹，每一种发射模式背后都是一套数学，而每一套数学都在回答同一个问题：<strong>如何在屏幕上创造"有美感的暴力"</strong>。这一节教你设计一个与发射模式解耦的子弹系统。</p>
    </ConceptBlock>

    <ConceptBlock icon="🕹️" title="一束光，一颗子弹——从《太空侵略者》的单发说起">
      <p>1978 年，太东（Taito）的《太空侵略者》定义了射击游戏的原始形态：玩家只有一颗子弹在屏幕上。打出去，飞到屏幕顶部，消失了——你才能再发第二颗。这不是故意限制你，是当时的硬件只支持屏幕上同时存在<strong>一颗玩家子弹</strong>。</p>
      <p>但就是这个"单发"限制，催生了射击游戏最核心的紧张感：<strong>每一发都要命中，打偏了就要等它飞出屏幕才能打下一发</strong>。当你面对一排越来越近的外星人、手中的唯一一发子弹正在慢悠悠往上飘——这就是游戏史上最早的"资源管理"体验。</p>
      <p>40 多年过去了，今天的射击游戏屏幕上可以同时有上百发子弹。但每颗子弹要回答的问题还是一样的：从哪来？往哪去？飞多快？打到什么算命中？</p>
    </ConceptBlock>

    <ConceptBlock icon="🔄" title="子弹生命周期：和 DOM 元素惊人相似的命运">
      <p>一颗子弹从"诞生"到"死亡"经历的七个阶段，和你熟悉的 DOM 元素生命周期几乎一一对应：</p>
      <table>
      ...
      <p>理解这种"创建→挂载→更新→卸载"的循环至关重要。游戏引擎每秒要跑 60 次这个循环，每次循环可能有几十颗子弹同时处于不同阶段——这和 Vue 的组件生命周期在本质上共享同一套设计哲学。</p>
...
destroy()          // → beforeUnmount / unmounted</pre>
    ...
    <ConceptBlock icon="🎯" title="发射模式：接口即契约">
      <p>单发、双发、三发、扇形、追踪——如果每种发射模式都写在 PlayerController 里，你的 <code>shoot()</code> 方法很快会变成一大坨 if/else。解决之道和你写 Vue 组件时一样：<strong>定义接口，注入实现</strong>。</p>
      <pre>// 单发
...
update(dt: number) {
  ...
  this.node.position = this.node.position.add(this.dir.multiplyScalar(this.speed * dt))
    ...
    <ConceptBlock icon="🔧" title="解耦设计：别管发射什么，只管怎么发射">
      <p>如果你写过 Vue，你一定用过 <code>&lt;slot&gt;</code>——父组件只管"这里有个槽位"，至于槽位里放什么内容，由使用者决定。FirePattern 接口就是子弹系统的"槽位"：</p>
      <div class="tip-box">
...
onKeyDown(key: KeyCode) {
  ...
  if (key === KeyCode.SPACE) {
    ...
    <ConceptBlock icon="🔗" title="课外延伸">
      <p>不要求掌握，但如果你对"子弹"这个话题意犹未尽：</p>
      <ul>
        <li><strong>ZUN 和《东方 Project》——一个人创造的弹幕帝国：</strong> 1997 年，一个叫太田顺也（ZUN）的日本大学生，用 C 语言独自开发了一款弹幕射击游戏《东方灵异传》。此后 25 年，他一个人包揽了编程、美术、音乐、关卡设计——推出了超过 25 款东方系列作品。东方的子弹排列不只是为了难，更是 ZUN 个人的一种<strong>几何美学表达</strong>：扇形弹、螺旋弹、交叉线阵、激光切割……每一张符卡都是一幅动态的图案。ZUN 在 2010 年的一次演讲中说："弹幕不是为了让玩家死，是为了让玩家在子弹的间隙里跳舞。"这句话定义了弹幕游戏的设计哲学。</li>
        <li><strong>CAVE 与弹幕游戏的黄金时代：</strong> 1995 年到 2012 年间，日本公司 CAVE（《怒首领蜂》《虫姬》《死亡微笑》）将弹幕射击推向了极致。CAVE 的子弹设计师被称为"弹幕职人"——他们不写代码，而是用专属的弹幕编辑器调教每一发子弹的轨迹、速度、颜色和排列。CAVE 的游戏里，子弹的排列在美学上堪比烟花表演，而在数学上则是从贝塞尔曲线延伸到极坐标方程。2012 年 CAVE 退出街机市场时，整个游戏界都在惋惜"一个艺术形式的终结"。</li>
        <li><strong>对象池为什么对子弹系统至关重要：</strong> 你的游戏一秒钟可能射出 30 发子弹，一发子弹存活 2 秒，峰值时屏幕上就有 60 个子弹节点。如果每发子弹都是 <code>instantiate()</code> 创建、<code>destroy()</code> 销毁，JS 引擎会在几秒内产生上千次 GC（垃圾回收）。每次 GC 触发都会暂停所有 JS 执行 10-50ms——画面就这么"卡"一下。Phase 15 会详细讲对象池怎么一劳永逸地解决这个问题。这就像你在 Vue 里用 <code>&lt;KeepAlive&gt;</code> 缓存组件而不是每次都重新 mount——同样的思想，不同的战场。</li>
      </ul>
    </ConceptBlock>
    ...
    <ConceptBlock icon="✅" title="自测清单">
      <p>学完这一节，你应该能回答：</p>
      <ol>
        <li>为什么子弹发射逻辑要和子弹本身解耦？以 Vue 组件设计（slot/props/动态组件）为参照，解释 FirePattern 接口的设计意图。当你想加第四种发射模式时，需要改动哪些现有代码？</li>
        <li>子弹生命周期和 DOM 元素（或 Vue 组件）生命周期的对应关系是什么？为什么游戏引擎需要特别关注"创建/销毁"环节的性能，而 Web 前端对此相对不那么敏感？</li>
        <li>圆形发射模式（一圈子弹同时向 360° 射出）要怎么实现？试着在脑子里写出 <code>getBullets</code> 的循环逻辑——别写代码，描述算法就行。</li>
        <li>为什么追踪弹的"追踪"逻辑要放在 Bullet 的 update 里，而不是放在 FirePattern.getBullets 里？如果把追踪逻辑也放进 getBullets，会出现什么问题？</li>
      </ol>
    </ConceptBlock>
  </PhaseLayout>
</template>
