...
<template>
  <PhaseLayout :phase="13" title="波次系统" duration="1-2 天">
    <ConceptBlock icon="🧭" title="本节定位">
      <p>1978 年，《太空侵略者》（Space Invaders）上市。玩家打完一波外星人，一波又来了——<strong>而且更快</strong>。没有过场动画，没有"新敌人登场"的提示，只有一个冷酷的事实：你还活着，那就再来一波，比你刚经历的那波更难。这个简单的"打波"设计，47 年后仍然是射击游戏的核心骨架。这一节我们要做的，就是把这种"一波接一波"的节奏感，用<strong>数据驱动</strong>的方式做成一个系统。</p>
    </ConceptBlock>

    <ConceptBlock icon="👾" title="Space Invaders 发明了什么？不只是波次">
      <p>《太空侵略者》经常被作为"第一款卷轴射击游戏"载入史册，但它对游戏设计最大的贡献是一个更微妙的东西：<strong>难度递增不是靠数据，是靠游戏机制本身</strong>。</p>
      <p>在 Space Invaders 里，没有"敌人变快了""敌人血量增加了"这些数值调整——所有敌人永远一枪死。那难度是怎么递增的？<strong>每一波剩下的敌人越少，它们移动得越快</strong>。前 90% 的敌人慢吞吞，最后一只外星光是在屏幕底部疯狂横移——你越接近胜利，越容易失败。</p>
      <p>这种"用机制本身制造紧张，而不是靠数值膨胀"的设计，是波次系统的最高智慧。你的波次系统也应该追求这个：<strong>每一波给玩家一种新的问题，而不是同样的问题乘以更大的数字</strong>。</p>
    </ConceptBlock>

    <ConceptBlock icon="📊" title="数据驱动：把波次配成 JSON——和 webpack config 一样的哲学">
      <p>假设你是策划。你写好了 5 波敌人的逻辑，交给程序员。程序员把它硬编码在 TypeScript 里。过两天你想微调：第 3 波少 3 只小兵，第 5 波间隔从 1.5 秒改成 1.8 秒——每次都要找程序员改代码、重新编译、重新发版。这不是开发效率问题，是<strong>迭代效率问题</strong>。</p>
      <p>就像 webpack.config.js 或 vite.config.ts 一样——把配置从代码中分离出来，让非程序员也能调参：</p>
      <div class="tip-box">
  ...
  { wave: 5, enemies: [  // Boss 关
    ...
    <ConceptBlock icon="🎵" title="波次节奏：高潮与呼吸——和 Vue Router 导航守卫一样的设计">
      <p>好的波次设计像音乐——有前奏、有高潮、有间奏。不要每波都是匀速变难。一个常见的节奏模型：</p>
...
Wave 6-7：恢复后再次增压
...</pre>
      <p>这种"在波次之间做决策"的逻辑，天然需要一个守卫机制：</p>
      <div class="tip-box">
    ...
    <ConceptBlock icon="📈" title="难度曲线：心流理论的数学表达">
      <p>波次系统的灵魂是难度曲线。四种经典的曲线，每种代表一种设计哲学：</p>
      <table>
      ...
      <p>1975 年，心理学家 Mihaly Csikszentmihalyi（奇克森米哈伊）提出了<strong>心流（Flow）理论</strong>：当挑战难度和你的能力刚好匹配时，人会进入一种忘记时间、完全沉浸在所做的事情中的状态。挑战太低→无聊。挑战太高→焦虑。刚好在你的能力边界上→心流。好的波次系统本质上就是在做这件事：<strong>让每一波都刚好在玩家的能力边界上，维持"差一点就死了但就是没死"的紧张感</strong>。这就是为什么你玩《只狼》时明明死了一百次，却觉得"不是游戏难，是我还不够好"——因为它每次都在心流区间里。</p>
    </ConceptBlock>
...
export class WaveManager extends Component {
  ...
  private spawnEnemy(type: string) {
    ...
    <ConceptBlock icon="🔗" title="课外延伸">
      <ul>
        <li><strong>Mihaly Csikszentmihalyi 的心流理论与游戏难度设计：</strong> 1975 年，芝加哥大学的心理学家 Csikszentmihalyi 在研究艺术家和运动员时发现，当一个人在做他"刚好能胜任但又需要全力以赴"的事情时，会进入一种极度专注、忘记时间流逝的状态——他称之为"心流"（Flow）。游戏设计师在 21 世纪初开始有意识地将心流理论应用到难度曲线设计中：让波次难度始终在玩家能力的 90%-110% 之间波动，既不让玩家无聊（太低），也不让玩家挫败（太高）。《半条命 2》的开篇关卡、《Celeste》的每一面、《哈迪斯》的房间难度——都是在"心流通道"上精心雕刻的。你的波次 JSON 本质上是一个心流通道的数学表达。</li>
        <li><strong>《黑暗之魂》的"平坦难度曲线"反例：</strong> 大多数 RPG 使用很陡的难度曲线——第一只史莱姆两刀秒，最终 Boss 要打 20 分钟。但 FromSoftware 的魂系列用的是一条<strong>几乎平坦的难度曲线</strong>。第一个 Boss（不死院恶魔）和第一只精英怪差不多难，甚至比中期某些区域更难——不是因为数值，是因为那时候的你还不会玩。魂系列的"变简单"不是靠数值膨胀，是靠<strong>你变强了</strong>。这是一种逆天的设计哲学：不是让游戏越来越难，是让你对自己能力的感觉越来越好。相比之下，放置类游戏（Cookie Clicker、Adventure Capitalist）用的是纯粹的指数曲线——前期秒秒钟十倍增长，后期等一天只涨 1%。两种极端，没有对错，只有适合不同玩家。</li>
        <li><strong>三消游戏的"随机难度"——老虎机心理学在关卡设计中的应用：</strong> King 公司（2012 年被 Activision 收购）的《Candy Crush Saga》有一个著名的设计：简单关和困难关交替出现，但困难关总是让你"差一两个步就过了"。这不是疏忽——是精确设计的。King 的数据团队发现，如果玩家连续失败 5 次，第 6 次在"只差一步就过"的时刻给一个付费道具的推送，转化率最高。这不是游戏设计，是行为心理学和 A/B 测试的结合。作为游戏开发者，你可以不同意这种做法，但你必须知道它的存在——因为你的波次系统同样可以操作"差一点就过"的体验。</li>
      </ul>
    </ConceptBlock>
    ...
    <ConceptBlock icon="✅" title="自测清单">
      <p>学完这一节，你应该能回答：</p>
      <ol>
        <li>为什么数据驱动（JSON 配置）比硬编码波次数据更好？从迭代效率（策划→程序员→编译→发版 vs 策划→改 JSON→运行）、团队分工、A/B 测试三个角度分析。</li>
        <li>心流理论如何指导你的难度曲线设计？假设你发现玩家在第 3 波大量流失（关闭游戏），从心流理论的角度，可能出了什么问题？你应该调整哪个参数？</li>
        <li>什么是一个"好"的难度曲线？如果你玩一个游戏，前 10 分钟都在秒杀一切，第 11 分钟突然死活过不去——问题出在哪里？应该怎么改？</li>
        <li>波次之间的"休息时间"为什么要单独配置？如果休息时间是固定的（比如永远是 3 秒），在 Boss 关之后和热身关之后，玩家的体验会有什么不同？</li>
      </ol>
    </ConceptBlock>
  </PhaseLayout>
</template>
