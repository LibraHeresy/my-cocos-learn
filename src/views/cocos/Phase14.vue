...
<template>
  <PhaseLayout :phase="14" title="道具系统" duration="1-2 天">
    <ConceptBlock icon="🧭" title="本节定位">
      <p>1985 年，马力欧吃了一个蘑菇——变大了。这是游戏史上最重要的道具。它没有教程、没有文字说明——你看到蘑菇，走过去，碰到了，变大了。<strong>道具设计的第一原则："让玩家本能地知道它有用。"</strong> 这一节我们要做的道具系统，同样追求这个原则：掉落有规律、效果有层次、组合有惊喜。</p>
    </ConceptBlock>

    <ConceptBlock icon="🍄" title="超级蘑菇教给游戏设计的一切">
      <p>马力欧的超级蘑菇堪称游戏史上最成功的道具设计——不是因为它"让角色变大"，是因为它用最少的视觉信息传达了最多的意思：</p>
      <ul>
        <li><strong>颜色鲜艳（红色黄点）：</strong> 在棕色砖块的背景下极其醒目——"我有好东西"</li>
        <li><strong>它会动（从砖块中跳出，沿地面滚动）：</strong> "你要追我"——给了玩家一个本能的动力：去碰它</li>
        <li><strong>碰到 = 变大：</strong> 不需要任何文字，"碰一下试试"是所有玩家面对蘑菇时的本能反应</li>
        <li><strong>变大后你更"大"了，但受击面积也更大了：</strong> 有得必有失——这才是成熟的道具设计</li>
      </ul>
      <p>宫本茂在设计这个蘑菇时说过一句话："最好的教学是让玩家自己发现。"你在设计道具时也请记住——不要让道具弹出一段文字解释自己。让它的外观、动效、音效本能地告诉玩家："捡我。"</p>
    </ConceptBlock>

    <ConceptBlock icon="🎁" title="三种效果类型：从即时到持续到累积">
      <p>道具有三种本质上不同的生效方式。理解它们的区别，是设计道具系统的基础：</p>
      <table>
    ...
    <ConceptBlock icon="🎲" title="掉落表：游戏世界的 CSS 特异性权重">
      <p>CSS 选择器的优先级不是均匀的——<code>#id</code> > <code>.class</code> > <code>element</code>，每个选择器有自己的"权重"。道具掉落也是同样的逻辑：不是所有道具应该有相同的掉落概率。我们需要一个<strong>权重系统</strong>：</p>
      <div class="tip-box">
...
function rollDrop(): string {
  ...
  return 'none'
      ...
      <p>权重随机比均匀随机灵活得多——调概率只需要改 weight 值，不需要改任何判断逻辑。加新道具也只需要在表里加一行。</p>
    </ConceptBlock>

    <ConceptBlock icon="🔄" title="道具叠加：CSS 自定义属性的级联逻辑">
      <p>多个道具同时作用于一个玩家时，怎么处理？这和 CSS 自定义属性的级联逻辑是一样的——"后到的覆盖先到的"是最简单的策略，但还有更多选择：</p>
      <div class="tip-box">
    ...
    <ConceptBlock icon="🔗" title="课外延伸">
      <ul>
        <li><strong>马力欧的蘑菇到底教了什么？——道具即教程：</strong> 1985 年《超级马力欧兄弟》的 1-1 关卡是世界公认的游戏教学设计典范。在你玩的第一个 30 秒里，宫本茂安排了三个关键教学：1) 一个蘑菇从砖块里跳出来——教会你"碰砖块有东西"；2) 那蘑菇在地面上滚——教会你"地上的东西可以追"；3) 你碰到了蘑菇，变大了——教会你"有东西是好的，碰到了就生效"。全程没有一行文字、没有一个弹窗。这种"道具即教程"的设计哲学至今仍然是所有游戏 UI/UX 设计师的必修课。你的道具设计也应该追求这个标准——不要写"按 E 键拾取"，让道具看起来就让人想走过去。</li>
        <li><strong>《暗黑破坏神》的随机词缀装备——概率的组合爆炸：</strong> 1996 年暴雪北方的《暗黑破坏神》发售时，没有人预见到它的装备系统会彻底改变整个游戏产业。暗黑的每件装备有 6-8 个词缀（属性加成），每个词缀有类别（力量、敏捷、暴击、吸血……）和数值范围（+1~+100），不同词缀之间可以组合——一件装备理论上能有数百万种组合。玩家为了"那个词缀完美搭配"愿意刷上千小时。这和你这节写的 DROP_TABLE 原理完全一样——区别只在于暗黑的掉落表挂了一万个条目，每个条目有更多维度的权重。2012 年《暗黑破坏神 3》的拍卖行灾难（暴雪试图用 RMT 经济来承接这套随机系统，结果玩崩了）更是给所有游戏开发者的警告：道具系统的设计会影响整个游戏的经济体系。</li>
        <li><strong>《以撒的结合》的道具组合爆炸——700 种道具的排列组合：</strong> 2011 年 Edmund McMillen 的《以撒的结合》以 700+ 种道具闻名。但它真正的设计奇迹不是数量——是<strong>道具之间的组合效果</strong>。比如"硫磺火"（射出激光）配上"三眼"（三发子弹），结果不是"激光+三发"，是三道激光；配上"科技"（电击枪），结果是激光变成电击鞭。每种组合都是手工设计的——这种组合爆炸是 Roguelike 游戏的核心乐趣，但也是测试的噩梦：700 种道具的配对组合数是 244,650 种，三件套的组合数就超过 5700 万。McMillen 的解决方案是"接受不完美"——有些组合非常弱，有些极其强，但"找到那个极强的组合"本身就是玩家的驱动力。你的道具系统不需要 700 种道具，但你需要理解这个设计哲学：不能让每种道具都只是"+10% 攻击力"——要给玩家组合的空间和惊喜。</li>
      </ul>
    </ConceptBlock>
...
eventBus.on('enemy-killed', (data) => {
  ...
  if (drop !== 'none') {
    ...
    <ConceptBlock icon="✅" title="自测清单">
      <p>学完这一节，你应该能回答：</p>
      <ol>
        <li>为什么马力欧的超级蘑菇被认为是"史上最佳道具设计"？它用了哪些设计技巧让玩家本能地知道"这个东西有用"？你的道具设计可以从中学到什么？</li>
        <li>即时生效（Bomb）、持续生效（Shield）和触发型（Extra Life）道具在代码实现上最关键的区别是什么？每种类型需要记录什么额外的状态信息？如何管理多个持续道具的同时存在？</li>
        <li>权重随机为什么比"均匀随机 + if/else 判断"更好？以 CSS 选择器优先级（specificity）为参照，解释权重系统的设计思路。</li>
        <li>如果玩家同时吃到两个护盾道具（一个是刚吃的还剩 3 秒，一个是刚掉落的），应该采用哪种叠加策略（覆盖、延长、叠加、互斥）？如果吃到两个不同颜色的护盾呢（火盾 + 冰盾）？</li>
      </ol>
    </ConceptBlock>
  </PhaseLayout>
</template>
