...
<template>
  <PhaseLayout :phase="12" title="敌机系统与行为设计" duration="2-3 天">
    <ConceptBlock icon="🧭" title="本节定位">
      <p>Pac-Man 的四个幽灵——<strong>Blinky 追你、Pinky 堵你、Inky 随机跑、Clyde 装傻</strong>。1980 年的游戏，4 个敌人，4 种 AI 行为，只有不到 1KB 的代码。它不需要讲一个宏大的背景故事；你只需要看 30 秒，就能从幽灵的颜色辨别谁在追你。这就是敌机设计的最高境界：<strong>用简单的规则创造出丰富的战术变化</strong>。这一节我们来实现这种"简单规则，复杂行为"的敌机系统。</p>
    </ConceptBlock>

    <ConceptBlock icon="👻" title="Pac-Man 的遗产：四个幽灵，四种性格">
      <p>Pac-Man（吃豆人）之所以 45 年来仍然被奉为游戏设计教科书，很大程度上是因为它的幽灵 AI。每一个幽灵有自己的行为模式，而且这些模式不靠随机数——靠<strong>确定性规则</strong>，这给了玩家可预测、可学习的对手：</p>
      <table>
      ...
      <p>注意到了吗？<strong>没有一个幽灵是"随机"的</strong>。每个幽灵的行为都有确定的数学规则，但四个规则叠加在一起，产生了看似混沌的追逐体验——这就是"涌现复杂度"（Emergent Complexity）。Pac-Man 的幽灵 AI 只用了不到 100 行汇编代码，但它创造的战术深度至今没有多少游戏能超越。</p>
    </ConceptBlock>

    <ConceptBlock icon="👾" title="敌机类型设计：从数据出发">
      <p>扩展到射击游戏，敌机设计的第一原则同样是：<strong>让玩家能"学会"每一种敌机</strong>。敌机的视觉特征要和它的行为绑定——看到大的就知道它难打，看到正弦波飞行的就知道它下一步会摆到左边：</p>
      <table>
    ...
    <ConceptBlock icon="〰️" title="移动轨迹：游戏世界的 CSS Keyframes">
      <p>如果你写过 CSS 动画，你对 <code>@keyframes</code> 一定不陌生——定义一组关键帧，浏览器帮你自动插值。敌机移动本质上就是定义一条<strong>路径</strong>，然后让引擎每帧沿路径推进：</p>
      <div class="tip-box">
...
pos = bezier(t, startPoint, control1, control2, endPoint)</pre>
    ...
    <ConceptBlock icon="🏭" title="敌机生成器：Vue 动态组件的游戏版本">
      <p>在一个关卡中，不同波次需要生成不同的敌机组合——有时是 10 只小型机直线冲锋，有时是 3 只中型机正弦波扫射，有时是 Boss 登场。这就像你在 Vue 里用 <code>&lt;component :is="currentEnemyType"&gt;</code> 动态切换组件一样，敌机生成器（Spawner）就是游戏的"动态组件加载器"：</p>
      <div class="tip-box">
...
export class EnemySpawner extends Component {
  ...
  private spawnWave() {
    ...
    <ConceptBlock icon="🧠" title="有限状态机：为什么敌机 AI 需要一个「大脑」">
      <p>上面 Boss 的阶段切换是最简单的 FSM（Finite State Machine）。但如果你用 if/else 在 update 里实现——</p>
...
update(dt) {
  if (this.hp > maxHp * 0.5) {
    ...
    this.moveChase(dt)
      ...
      <p>看起来还行？等到 Boss 有 8 个状态、每个状态有多种子行为、状态之间还有过渡动画——那团 if/else 会让你痛不欲生。FSM 的本质是把"在什么状态下做什么事"和"状态之间怎么切换"分开。</p>
      <div class="tip-box">
    ...
    <ConceptBlock icon="🔗" title="课外延伸">
      <ul>
        <li><strong>Pac-Man 幽灵 AI 的"涌现复杂度"哲学：</strong> 游戏设计师岩谷彻（Toru Iwatani）在设计 Pac-Man 时面临一个限制——1980 年的 Namco 街机版只有 16KB ROM。他不能写复杂的 AI，于是他想了一个办法：给四个幽灵每人一个"简单的追逐规则"，让玩家的大脑去"建构"这些幽灵的性格。结果出乎所有人意料——玩家真的给幽灵起了名字（最初没有名字，是玩家叫出来的），真的相信每个幽灵有自己的性格。这就是游戏 AI 设计的最高智慧：不是写出聪明的 AI，而是创造出<strong>让玩家觉得 AI 聪明</strong>的简单规则。</li>
        <li><strong>《黑暗之魂》的敌怪放置：不是随机的，是"雕刻的"：</strong> FromSoftware 的关卡设计师宫崎英高有一个著名的工作方法：他亲自走到游戏引擎中，在一个拐角处放一个敌人，然后测试——死了。调整位置，再测试——还是死了，但这次差一点就躲开了。再调——"对，就是这个位置"。黑魂的敌人位置不是随机分布的，是一个一个"雕刻"出来的。每一个敌人的出现位置、朝向、警觉距离，都是为了让玩家"刚好在极限上"。这和射击游戏的敌机生成是同一个道理：敌人在哪出现、什么时机出现、以什么轨迹出现，决定了玩家的情绪曲线。</li>
        <li><strong>音乐游戏的"音符谱面"：敌机生成的另一个维度：</strong> 在 Beatmania、Guitar Hero、太鼓达人等音乐游戏中，音符不是随机落下的——每一首歌的谱面是设计师<strong>手工编排</strong>的。好的谱面让按键和音乐融为一体；坏的谱面让人按对了也觉得别扭。这和你的敌机生成系统本质相同：敌机的出现时机、类型组合、轨迹变化，应该像一首歌的编曲——有前奏、有高潮、有留白。当你设计波次时（Phase 13 会展开），试着闭上眼睛听一遍波次的"节奏"——如果它听起来像一首没有副歌的歌，那就需要重写。</li>
      </ul>
    </ConceptBlock>
    ...
    <ConceptBlock icon="✅" title="自测清单">
      <p>学完这一节，你应该能回答：</p>
      <ol>
        <li>Pac-Man 的四个幽灵用了什么设计技巧，让玩家觉得它们"有性格"，而实际上它们的行为都是确定性规则？这和你的敌机系统设计有什么共通之处？</li>
        <li>CSS <code>@keyframes</code> 动画和敌机移动轨迹在概念上有什么相似之处？为什么游戏里不能用纯 CSS 来做敌机移动（哪怕游戏跑在浏览器里）？——提示：从"确定性路径"和"动态响应"的角度思考。</li>
        <li>什么时候用简单的直线/正弦运动就够了，什么时候需要贝塞尔曲线或追踪逻辑？从游戏节奏和小怪 vs Boss 的区分来分析。</li>
        <li>有限状态机（FSM）对敌机 AI 有什么帮助？以 Vue Router 或 Pinia 为参照，解释为什么 FSM 是管理"状态+行为"组合的最佳方式。</li>
      </ol>
    </ConceptBlock>
  </PhaseLayout>
</template>
