...
onMounted(() => {
  try { const raw = localStorage.getItem(STORAGE_KEY); if (raw) { const p = JSON.parse(raw); checklist.value = p[2]?.items || {}; completed.value = p[2]?.done || false } } catch {}
})
...
function save() {
  try {
    ...
    all[2] = { items: checklist.value, done: completed.value }
...
<template>
  <div class="day-page">
    <div class="day-header-badge">Day 2</div>
    <h1>{{ day.title }}</h1>
    ...
    <nav class="day-nav">
      <RouterLink v-if="2 > 1" to="/speedrun/day/1" class="nav-btn">← Day 1</RouterLink>
      <span v-else class="nav-btn disabled">← 没有更早的了</span>
      <RouterLink v-if="2 < 30" to="/speedrun/day/3" class="nav-btn">Day 3 →</RouterLink>
