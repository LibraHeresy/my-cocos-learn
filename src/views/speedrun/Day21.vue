...
onMounted(() => {
  try { const raw = localStorage.getItem(STORAGE_KEY); if (raw) { const p = JSON.parse(raw); checklist.value = p[21]?.items || {}; completed.value = p[21]?.done || false } } catch {}
})
...
function save() {
  try {
    ...
    all[21] = { items: checklist.value, done: completed.value }
...
<template>
  <div class="day-page">
    <div class="day-header-badge">Day 21</div>
    <h1>{{ day.title }}</h1>
    ...
    <nav class="day-nav">
      <RouterLink v-if="21 > 1" to="/speedrun/day/20" class="nav-btn">← Day 20</RouterLink>
      <span v-else class="nav-btn disabled">← 没有更早的了</span>
      <RouterLink v-if="21 < 30" to="/speedrun/day/22" class="nav-btn">Day 22 →</RouterLink>
