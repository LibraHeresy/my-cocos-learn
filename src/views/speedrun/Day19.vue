...
onMounted(() => {
  try { const raw = localStorage.getItem(STORAGE_KEY); if (raw) { const p = JSON.parse(raw); checklist.value = p[19]?.items || {}; completed.value = p[19]?.done || false } } catch {}
})
...
function save() {
  try {
    ...
    all[19] = { items: checklist.value, done: completed.value }
...
<template>
  <div class="day-page">
    <div class="day-header-badge">Day 19</div>
    <h1>{{ day.title }}</h1>
    ...
    <nav class="day-nav">
      <RouterLink v-if="19 > 1" to="/speedrun/day/18" class="nav-btn">← Day 18</RouterLink>
      <span v-else class="nav-btn disabled">← 没有更早的了</span>
      <RouterLink v-if="19 < 30" to="/speedrun/day/20" class="nav-btn">Day 20 →</RouterLink>
