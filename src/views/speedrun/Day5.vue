...
onMounted(() => {
  try { const raw = localStorage.getItem(STORAGE_KEY); if (raw) { const p = JSON.parse(raw); checklist.value = p[5]?.items || {}; completed.value = p[5]?.done || false } } catch {}
})
...
function save() {
  try {
    ...
    all[5] = { items: checklist.value, done: completed.value }
...
<template>
  <div class="day-page">
    <div class="day-header-badge">Day 5</div>
    <h1>{{ day.title }}</h1>
    ...
    <nav class="day-nav">
      <RouterLink v-if="5 > 1" to="/speedrun/day/4" class="nav-btn">← Day 4</RouterLink>
      <span v-else class="nav-btn disabled">← 没有更早的了</span>
      <RouterLink v-if="5 < 30" to="/speedrun/day/6" class="nav-btn">Day 6 →</RouterLink>
