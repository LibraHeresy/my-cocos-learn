...
onMounted(() => {
  try { const raw = localStorage.getItem(STORAGE_KEY); if (raw) { const p = JSON.parse(raw); checklist.value = p[28]?.items || {}; completed.value = p[28]?.done || false } } catch {}
})
...
function save() {
  try {
    ...
    all[28] = { items: checklist.value, done: completed.value }
...
<template>
  <div class="day-page">
    <div class="day-header-badge">Day 28</div>
    <h1>{{ day.title }}</h1>
    ...
    <nav class="day-nav">
      <RouterLink v-if="28 > 1" to="/speedrun/day/27" class="nav-btn">← Day 27</RouterLink>
      <span v-else class="nav-btn disabled">← 没有更早的了</span>
      <RouterLink v-if="28 < 30" to="/speedrun/day/29" class="nav-btn">Day 29 →</RouterLink>
