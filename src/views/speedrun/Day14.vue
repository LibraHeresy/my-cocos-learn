...
onMounted(() => {
  try { const raw = localStorage.getItem(STORAGE_KEY); if (raw) { const p = JSON.parse(raw); checklist.value = p[14]?.items || {}; completed.value = p[14]?.done || false } } catch {}
})
...
function save() {
  try {
    ...
    all[14] = { items: checklist.value, done: completed.value }
...
<template>
  <div class="day-page">
    <div class="day-header-badge">Day 14</div>
    <h1>{{ day.title }}</h1>
    ...
    <nav class="day-nav">
      <RouterLink v-if="14 > 1" to="/speedrun/day/13" class="nav-btn">← Day 13</RouterLink>
      <span v-else class="nav-btn disabled">← 没有更早的了</span>
      <RouterLink v-if="14 < 30" to="/speedrun/day/15" class="nav-btn">Day 15 →</RouterLink>
