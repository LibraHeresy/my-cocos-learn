...
onMounted(() => {
  try { const raw = localStorage.getItem(STORAGE_KEY); if (raw) { const p = JSON.parse(raw); checklist.value = p[1]?.items || {}; completed.value = p[1]?.done || false } } catch {}
})

function toggle(i: number) {
  checklist.value[i] = !checklist.value[i]
  save()
}
...
function save() {
  try {
    ...
    all[1] = { items: checklist.value, done: completed.value }
...
<template>
  <div class="day-page">
    <div class="day-header-badge">Day 1</div>
    <h1>{{ day.title }}</h1>
    ...
    <nav class="day-nav">
      <span class="nav-btn disabled">← 没有更早的了</span>
      <RouterLink to="/speedrun/day/2" class="nav-btn">Day 2 →</RouterLink>
    </nav>
  </div>
</template>
