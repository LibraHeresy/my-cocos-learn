...
onMounted(() => {
  try { const raw = localStorage.getItem(STORAGE_KEY); if (raw) { const p = JSON.parse(raw); checklist.value = p[30]?.items || {}; completed.value = p[30]?.done || false } } catch {}
})
...
function save() {
  try {
    ...
    all[30] = { items: checklist.value, done: completed.value }
...
<template>
  <div class="day-page">
    <div class="day-header-badge">Day 30</div>
    <h1>{{ day.title }}</h1>
    ...
    <nav class="day-nav">
      <RouterLink v-if="30 > 1" to="/speedrun/day/29" class="nav-btn">← Day 29</RouterLink>
      <span v-else class="nav-btn disabled">← 没有更早的了</span>
      <RouterLink v-if="30 < 30" to="/speedrun/day/31" class="nav-btn">Day 31 →</RouterLink>
