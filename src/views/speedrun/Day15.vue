...
onMounted(() => {
  try { const raw = localStorage.getItem(STORAGE_KEY); if (raw) { const p = JSON.parse(raw); checklist.value = p[15]?.items || {}; completed.value = p[15]?.done || false } } catch {}
})
...
function save() {
  try {
    ...
    all[15] = { items: checklist.value, done: completed.value }
...
<template>
  <div class="day-page">
    <div class="day-header-badge">Day 15</div>
    <h1>{{ day.title }}</h1>
    ...
    <nav class="day-nav">
      <RouterLink v-if="15 > 1" to="/speedrun/day/14" class="nav-btn">← Day 14</RouterLink>
      <span v-else class="nav-btn disabled">← 没有更早的了</span>
      <RouterLink v-if="15 < 30" to="/speedrun/day/16" class="nav-btn">Day 16 →</RouterLink>
