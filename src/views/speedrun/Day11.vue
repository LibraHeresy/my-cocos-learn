...
onMounted(() => {
  try { const raw = localStorage.getItem(STORAGE_KEY); if (raw) { const p = JSON.parse(raw); checklist.value = p[11]?.items || {}; completed.value = p[11]?.done || false } } catch {}
})
...
function save() {
  try {
    ...
    all[11] = { items: checklist.value, done: completed.value }
...
<template>
  <div class="day-page">
    <div class="day-header-badge">Day 11</div>
    <h1>{{ day.title }}</h1>
    ...
    <nav class="day-nav">
      <RouterLink v-if="11 > 1" to="/speedrun/day/10" class="nav-btn">← Day 10</RouterLink>
      <span v-else class="nav-btn disabled">← 没有更早的了</span>
      <RouterLink v-if="11 < 30" to="/speedrun/day/12" class="nav-btn">Day 12 →</RouterLink>
