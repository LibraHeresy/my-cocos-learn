...
onMounted(() => {
  try { const raw = localStorage.getItem(STORAGE_KEY); if (raw) { const p = JSON.parse(raw); checklist.value = p[20]?.items || {}; completed.value = p[20]?.done || false } } catch {}
})
...
function save() {
  try {
    ...
    all[20] = { items: checklist.value, done: completed.value }
...
<template>
  <div class="day-page">
    <div class="day-header-badge">Day 20</div>
    <h1>{{ day.title }}</h1>
    ...
    <nav class="day-nav">
      <RouterLink v-if="20 > 1" to="/speedrun/day/19" class="nav-btn">← Day 19</RouterLink>
      <span v-else class="nav-btn disabled">← 没有更早的了</span>
      <RouterLink v-if="20 < 30" to="/speedrun/day/21" class="nav-btn">Day 21 →</RouterLink>
