...
onMounted(() => {
  try { const raw = localStorage.getItem(STORAGE_KEY); if (raw) { const p = JSON.parse(raw); checklist.value = p[24]?.items || {}; completed.value = p[24]?.done || false } } catch {}
})
...
function save() {
  try {
    ...
    all[24] = { items: checklist.value, done: completed.value }
...
<template>
  <div class="day-page">
    <div class="day-header-badge">Day 24</div>
    <h1>{{ day.title }}</h1>
    ...
    <nav class="day-nav">
      <RouterLink v-if="24 > 1" to="/speedrun/day/23" class="nav-btn">← Day 23</RouterLink>
      <span v-else class="nav-btn disabled">← 没有更早的了</span>
      <RouterLink v-if="24 < 30" to="/speedrun/day/25" class="nav-btn">Day 25 →</RouterLink>
