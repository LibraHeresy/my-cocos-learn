...
onMounted(() => {
  try { const raw = localStorage.getItem(STORAGE_KEY); if (raw) { const p = JSON.parse(raw); checklist.value = p[7]?.items || {}; completed.value = p[7]?.done || false } } catch {}
})
...
function save() {
  try {
    ...
    all[7] = { items: checklist.value, done: completed.value }
...
<template>
  <div class="day-page">
    <div class="day-header-badge">Day 7</div>
    <h1>{{ day.title }}</h1>
    ...
    <nav class="day-nav">
      <RouterLink v-if="7 > 1" to="/speedrun/day/6" class="nav-btn">← Day 6</RouterLink>
      <span v-else class="nav-btn disabled">← 没有更早的了</span>
      <RouterLink v-if="7 < 30" to="/speedrun/day/8" class="nav-btn">Day 8 →</RouterLink>
