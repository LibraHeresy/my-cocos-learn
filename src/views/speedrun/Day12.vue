...
onMounted(() => {
  try { const raw = localStorage.getItem(STORAGE_KEY); if (raw) { const p = JSON.parse(raw); checklist.value = p[12]?.items || {}; completed.value = p[12]?.done || false } } catch {}
})
...
function save() {
  try {
    ...
    all[12] = { items: checklist.value, done: completed.value }
...
<template>
  <div class="day-page">
    <div class="day-header-badge">Day 12</div>
    <h1>{{ day.title }}</h1>
    ...
    <nav class="day-nav">
      <RouterLink v-if="12 > 1" to="/speedrun/day/11" class="nav-btn">← Day 11</RouterLink>
      <span v-else class="nav-btn disabled">← 没有更早的了</span>
      <RouterLink v-if="12 < 30" to="/speedrun/day/13" class="nav-btn">Day 13 →</RouterLink>
