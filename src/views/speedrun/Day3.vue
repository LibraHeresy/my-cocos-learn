...
onMounted(() => {
  try { const raw = localStorage.getItem(STORAGE_KEY); if (raw) { const p = JSON.parse(raw); checklist.value = p[3]?.items || {}; completed.value = p[3]?.done || false } } catch {}
})
...
function save() {
  try {
    ...
    all[3] = { items: checklist.value, done: completed.value }
...
<template>
  <div class="day-page">
    <div class="day-header-badge">Day 3</div>
    <h1>{{ day.title }}</h1>
    ...
    <nav class="day-nav">
      <RouterLink v-if="3 > 1" to="/speedrun/day/2" class="nav-btn">← Day 2</RouterLink>
      <span v-else class="nav-btn disabled">← 没有更早的了</span>
      <RouterLink v-if="3 < 30" to="/speedrun/day/4" class="nav-btn">Day 4 →</RouterLink>
