...
onMounted(() => {
  try { const raw = localStorage.getItem(STORAGE_KEY); if (raw) { const p = JSON.parse(raw); checklist.value = p[8]?.items || {}; completed.value = p[8]?.done || false } } catch {}
})
...
function save() {
  try {
    ...
    all[8] = { items: checklist.value, done: completed.value }
...
<template>
  <div class="day-page">
    <div class="day-header-badge">Day 8</div>
    <h1>{{ day.title }}</h1>
    ...
    <nav class="day-nav">
      <RouterLink v-if="8 > 1" to="/speedrun/day/7" class="nav-btn">← Day 7</RouterLink>
      <span v-else class="nav-btn disabled">← 没有更早的了</span>
      <RouterLink v-if="8 < 30" to="/speedrun/day/9" class="nav-btn">Day 9 →</RouterLink>
