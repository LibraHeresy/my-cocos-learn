...
onMounted(() => {
  try { const raw = localStorage.getItem(STORAGE_KEY); if (raw) { const p = JSON.parse(raw); checklist.value = p[25]?.items || {}; completed.value = p[25]?.done || false } } catch {}
})
...
function save() {
  try {
    ...
    all[25] = { items: checklist.value, done: completed.value }
...
<template>
  <div class="day-page">
    <div class="day-header-badge">Day 25</div>
    <h1>{{ day.title }}</h1>
    ...
    <nav class="day-nav">
      <RouterLink v-if="25 > 1" to="/speedrun/day/24" class="nav-btn">← Day 24</RouterLink>
      <span v-else class="nav-btn disabled">← 没有更早的了</span>
      <RouterLink v-if="25 < 30" to="/speedrun/day/26" class="nav-btn">Day 26 →</RouterLink>
