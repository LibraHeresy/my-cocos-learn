...
onMounted(() => {
  try { const raw = localStorage.getItem(STORAGE_KEY); if (raw) { const p = JSON.parse(raw); checklist.value = p[9]?.items || {}; completed.value = p[9]?.done || false } } catch {}
})
...
function save() {
  try {
    ...
    all[9] = { items: checklist.value, done: completed.value }
...
<template>
  <div class="day-page">
    <div class="day-header-badge">Day 9</div>
    <h1>{{ day.title }}</h1>
    ...
    <nav class="day-nav">
      <RouterLink v-if="9 > 1" to="/speedrun/day/8" class="nav-btn">← Day 8</RouterLink>
      <span v-else class="nav-btn disabled">← 没有更早的了</span>
      <RouterLink v-if="9 < 30" to="/speedrun/day/10" class="nav-btn">Day 10 →</RouterLink>
