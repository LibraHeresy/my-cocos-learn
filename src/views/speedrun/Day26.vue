...
onMounted(() => {
  try { const raw = localStorage.getItem(STORAGE_KEY); if (raw) { const p = JSON.parse(raw); checklist.value = p[26]?.items || {}; completed.value = p[26]?.done || false } } catch {}
})
...
function save() {
  try {
    ...
    all[26] = { items: checklist.value, done: completed.value }
...
<template>
  <div class="day-page">
    <div class="day-header-badge">Day 26</div>
    <h1>{{ day.title }}</h1>
    ...
    <nav class="day-nav">
      <RouterLink v-if="26 > 1" to="/speedrun/day/25" class="nav-btn">← Day 25</RouterLink>
      <span v-else class="nav-btn disabled">← 没有更早的了</span>
      <RouterLink v-if="26 < 30" to="/speedrun/day/27" class="nav-btn">Day 27 →</RouterLink>
