...
onMounted(() => {
  try { const raw = localStorage.getItem(STORAGE_KEY); if (raw) { const p = JSON.parse(raw); checklist.value = p[27]?.items || {}; completed.value = p[27]?.done || false } } catch {}
})
...
function save() {
  try {
    ...
    all[27] = { items: checklist.value, done: completed.value }
...
<template>
  <div class="day-page">
    <div class="day-header-badge">Day 27</div>
    <h1>{{ day.title }}</h1>
    ...
    <nav class="day-nav">
      <RouterLink v-if="27 > 1" to="/speedrun/day/26" class="nav-btn">← Day 26</RouterLink>
      <span v-else class="nav-btn disabled">← 没有更早的了</span>
      <RouterLink v-if="27 < 30" to="/speedrun/day/28" class="nav-btn">Day 28 →</RouterLink>
