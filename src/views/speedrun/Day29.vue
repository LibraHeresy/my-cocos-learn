...
onMounted(() => {
  try { const raw = localStorage.getItem(STORAGE_KEY); if (raw) { const p = JSON.parse(raw); checklist.value = p[29]?.items || {}; completed.value = p[29]?.done || false } } catch {}
})
...
function save() {
  try {
    ...
    all[29] = { items: checklist.value, done: completed.value }
...
<template>
  <div class="day-page">
    <div class="day-header-badge">Day 29</div>
    <h1>{{ day.title }}</h1>
    ...
    <nav class="day-nav">
      <RouterLink v-if="29 > 1" to="/speedrun/day/28" class="nav-btn">← Day 28</RouterLink>
      <span v-else class="nav-btn disabled">← 没有更早的了</span>
      <RouterLink v-if="29 < 30" to="/speedrun/day/30" class="nav-btn">Day 30 →</RouterLink>
