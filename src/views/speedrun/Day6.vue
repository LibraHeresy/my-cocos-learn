...
onMounted(() => {
  try { const raw = localStorage.getItem(STORAGE_KEY); if (raw) { const p = JSON.parse(raw); checklist.value = p[6]?.items || {}; completed.value = p[6]?.done || false } } catch {}
})
...
function save() {
  try {
    ...
    all[6] = { items: checklist.value, done: completed.value }
...
<template>
  <div class="day-page">
    <div class="day-header-badge">Day 6</div>
    <h1>{{ day.title }}</h1>
    ...
    <nav class="day-nav">
      <RouterLink v-if="6 > 1" to="/speedrun/day/5" class="nav-btn">← Day 5</RouterLink>
      <span v-else class="nav-btn disabled">← 没有更早的了</span>
      <RouterLink v-if="6 < 30" to="/speedrun/day/7" class="nav-btn">Day 7 →</RouterLink>
